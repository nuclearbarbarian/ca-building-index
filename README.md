...
Built in the Penney Design System — 1940s trade journal aesthetic.
...
Run the bundled scraper to generate fee data for all 121 cities:
```bash
npm run scrape        # researched + estimated data
npm run scrape:live   # also attempts live fetch from known fee schedule URLs
```
Then click **"📂 Load Fee Data"** in the header to upload `output/results.json`.
...
├── scraper/
│   └── scrape.js        ← fee data scraper (121 cities)
├── output/
│   └── results.json     ← generated by npm run scrape (gitignored)
└── src/
    ├── main.jsx
